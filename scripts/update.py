...
def update():
    ...
    if len(os.listdir(f"{sys.path[0]}/ml_models/active_models/")) != 0:
        move_models(f"{sys.path[0]}/ml_models/active_models/", f"{sys.path[0]}/ml_models/old_models/")
    # New to Active
    move_models(f"{sys.path[0]}/ml_models/new_models/", f"{sys.path[0]}/ml_models/active_models/")


def restore():
    # Old to active
    if len(os.listdir(f"{sys.path[0]}/ml_models/old_models/")) != 0:
        move_models(f"{sys.path[0]}/ml_models/old_models/", f"{sys.path[0]}/ml_models/active_models/")
    else:
        print(colored("No old AI's to restore.", "yellow"))
        return


def move_models(source_dir, target_dir):
